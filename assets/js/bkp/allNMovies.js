let allnMovieContainer = document.getElementById('allnMovieContainer');
let AllPnMovie = [];
...
function showAllnMovies() {
    ...
    allnMovieContainer.innerHTML = HTML
}
function getAllNMovies(){
    fetch("http://moviein.test/api/list_movies.php?sort_by=year&order_by=desc&page=1").then(res => {
        if (!res.ok){
