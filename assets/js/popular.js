...
console.log('alltime.php' === lastRoute)
if ('alltime.php' === lastRoute){
    document.getElementById('popular-nav').classList.add('nav-hov-active');
}
...
function getRecentMovies(){
    fetch(window.api_url+'/api/list_movies.php?page='+pageNo+'&sort_by=rating').then(res => {
        if (!res.ok){
            throw Error;
        }
        return res.json();
